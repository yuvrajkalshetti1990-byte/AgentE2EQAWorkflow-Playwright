...
test.describe('PIM — New Employee Onboarding — Happy Path', () => {
  ...
  test('TC-SCRUM-14-HP-03: Add employee with Create Login Details enabled (Status: Enabled)', async ({ page }) => {
    const ts = Date.now();
    const firstName = `LoginEn${ts}`;
    const lastName = 'SCRUM14';
    const username = `emp.en.${ts}`;
    ...
    await expect(page.locator('.oxd-input-group').filter({ hasText: /^Password$/ }).locator('input')).toBeVisible();
    await expect(page.locator('.oxd-input-group').filter({ hasText: 'Confirm Password' }).locator('input')).toBeVisible();
    ...
    // 7. Verify Status defaults to Enabled
